...
rows = []
File.foreach("data/#{log_folder}/depth.txt") do |line|  # read in one line/row at a time
...
lith_data = ''
lith = File.open("data/#{log_folder}/lith.txt", "r") do |file|
...
new_data.each do |lith|
	...
	Lithology.create(
		depth:							depth,
		felsite:						syms.include?('F') ? perc[syms.index('F')] : 0,
...
min = File.open("data/#{log_folder}/min.txt", "r") do |file|
  min_data = file.read.chomp
end

min_arr = min_data.split("\r\n")
# Qtz,Calcite,Pyrite,Epidote,pYrhotite,chLorite,aXinite,Actinolite,Tourmaline
...
min_arr.each do |line|
    ...
    epidote:		min_conversion[line_arr[4]],
    pyrrhotite:	min_conversion[line_arr[5]],
    chlorite:		min_conversion[line_arr[6]],
    axinite:		min_conversion[line_arr[7]],
    actinolite:	min_conversion[line_arr[8]],
    tourmaline:	min_conversion[line_arr[9]],
    well_id:		seed_well_id
	)
end

##################################################################
# SummaryDescription.destroy_all
i = 0
10.times do
	SummaryDescription.create(
		depth: 1000 + i,
